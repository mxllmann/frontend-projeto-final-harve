...
export default function QuizExecution() {
  ...
  const { questions, setQuestions, answers, setAnswer, resetQuiz } = useQuiz();
    ...
    <div className="min-h-screen bg-gradient-to-br  from-indigo-500 to-purple-600 p-6 text-white">
      <div className="max-w-3xl mx-auto bg-white text-black p-6 rounded-xl shadow-md">
        ...
        <div className="flex justify-between mt-6">
          <button onClick={handleBack} disabled={currentIndex === 0} className="bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300 disabled:opacity-50">
            Voltar
          </button>
          {currentIndex < questions.length - 1 ? (
            <button onClick={handleNext} className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700">
              Próxima
            </button>
          ) : (
